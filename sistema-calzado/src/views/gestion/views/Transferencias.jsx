    ...
    <>
        ...
        <Card className="mb-4" padding="md">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Icon d={ICONS.alert} size={16} className="text-amber-700" />
              <h2 className="text-[15px] text-foreground" style={{ fontWeight: 600 }}>
      ...
      <Card padding="sm" className="mb-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {[
            { label: 'Hoy',    fn: () => { const h = new Date().toISOString().slice(0,10); setFiltroDesde(h); setFiltroHasta(h); } },
            { label: 'Semana', fn: () => { const h = new Date(); const lun = new Date(h); lun.setDate(h.getDate() - ((h.getDay()+6)%7)); setFiltroDesde(lun.toISOString().slice(0,10)); setFiltroHasta(h.toISOString().slice(0,10)); } },
            { label: 'Mes',    fn: () => { const h = new Date(); setFiltroDesde(new Date(h.getFullYear(), h.getMonth(), 1).toISOString().slice(0,10)); setFiltroHasta(h.toISOString().slice(0,10)); } },
            { label: 'Todo',   fn: () => { setFiltroDesde(''); setFiltroHasta(''); } },
          ].map(pr => (
            <button
              key={pr.label}
              onClick={pr.fn}
    ...
    >
      ...
      {onMarcarReembolso && (
        <button
          ...
          className="px-3 py-1.5 rounded-lg text-xs bg-card border text-foreground hover:bg-muted/30 transition-colors flex-shrink-0"
...
function TransferenciaRow({ transferencia, onClick }) {
  ...
  return (
    <div
      className={`flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-muted/30 transition-colors cursor-pointer ${anulada ? 'opacity-50' : ''}`}
      onClick={onClick}
    >
      <div className="w-9 flex-shrink-0 text-[11px] text-muted-foreground fin-num text-center" style={{ fontWeight: 400 }}>
        {formatDate(t.fecha)}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        {t.concepto && (
          <p className="text-[11px] text-muted-foreground truncate mt-0.5" style={{ fontWeight: 400 }}>
            {t.concepto}
          </p>
        )}
      </div>
      ...
      <div className="text-right flex-shrink-0">
        <p className={`text-sm fin-num ${anulada ? 'line-through text-muted-foreground' : 'text-foreground'}`}
           style={{ fontWeight: 500 }}>
          {formatMoney(t.monto)}
        </p>
      </div>
    ...
    <div>
      ...
      <div className="flex items-center justify-center gap-4 py-4 border-b border-border/50">
        <div className="text-center flex-1 min-w-0">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1" style={{ fontWeight: 500 }}>Desde</p>
          <p className="text-sm text-foreground truncate" style={{ fontWeight: 500 }}>{t.origen?.nombre || '—'}</p>
          {t.origen?.alias && (
            <p className="text-[11px] text-muted-foreground" style={{ fontWeight: 400 }}>{t.origen.alias}</p>
          )}
        </div>

        <div className="flex flex-col items-center flex-shrink-0">
          <Icon d={ICONS.arrowRight} size={20} className="text-muted-foreground" />
          <p className="text-xl text-foreground fin-num mt-1" style={{ fontWeight: 600, letterSpacing: '-0.02em' }}>
            {formatMoney(t.monto)}
          </p>
        </div>

        <div className="text-center flex-1 min-w-0">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1" style={{ fontWeight: 500 }}>Hacia</p>
          <p className="text-sm text-foreground truncate" style={{ fontWeight: 500 }}>{t.destino?.nombre || '—'}</p>
          {t.destino?.alias && (
            <p className="text-[11px] text-muted-foreground" style={{ fontWeight: 400 }}>{t.destino.alias}</p>
          )}
        </div>
      </div>
      ...
      {t.notas && (
        <div className="mt-4">
          ...
          <p className="text-sm text-foreground bg-muted/30 rounded-lg p-3" style={{ fontWeight: 400 }}>{t.notas}</p>
        </div>
      )}

      {/* Acciones */}
      {!anulada && (
        <div className="flex flex-col gap-2 mt-6 pt-4 border-t border-border/50">
          ...
          {puedeAnular && t.reembolsado && (
            <p className="text-[11px] text-muted-foreground text-center" style={{ fontWeight: 400 }}>
...
function DetalleField({ label, value }) {
  return (
    <div className="flex items-center justify-between py-1.5">
      ...
      <span className="text-sm text-foreground fin-num text-right" style={{ fontWeight: 500 }}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="bg-muted/30 rounded-lg p-3 mb-4">
        <p className="text-xs text-muted-foreground uppercase tracking-wider mb-2" style={{ fontWeight: 500 }}>Préstamo original</p>
        <div className="flex items-center justify-between">
          <p className="text-sm text-foreground" style={{ fontWeight: 500 }}>
            {prestamo.origen?.nombre} <span className="text-muted-foreground">→</span> {prestamo.destino?.nombre}
          </p>
          <p className="text-base text-foreground fin-num" style={{ fontWeight: 600 }}>{formatMoney(prestamo.monto)}</p>
        </div>
        {prestamo.concepto && (
          <p className="text-[11px] text-muted-foreground mt-1" style={{ fontWeight: 400 }}>{prestamo.concepto}</p>
        )}
        <p className="text-[11px] text-muted-foreground mt-1" style={{ fontWeight: 400 }}>
      ...
      <div className="border border-border rounded-lg p-3 mb-4">
        <p className="text-xs text-muted-foreground uppercase tracking-wider mb-3" style={{ fontWeight: 500 }}>
      ...
      {!crearMovimiento && (
        <div className="p-3 rounded-lg bg-amber-50/40 border border-amber-200 mb-4">
