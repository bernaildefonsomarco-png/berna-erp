...
import { RapidoProvider, useRapido } from './RapidoContext';
...
function Header() {
  const { usuario, logout } = useRapido();
...
function AppShell() {
  const { refrescarCuentas, cuentasCargadas } = useRapido();
...
export default function RapidoLayout({ usuario, logout }) {
  return (
    <RapidoProvider usuario={usuario} logout={logout}>
      <AppShell />
    </RapidoProvider>
  );
}
