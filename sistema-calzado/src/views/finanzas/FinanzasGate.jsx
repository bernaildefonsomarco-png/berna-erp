...
/* ──────────────────────────────────────────────────────────────────────────
   FinanzasGate
   ──────────────────────────────────────────────────────────────────────────
   Portero del módulo Finanzas. Recibe el PIN, valida contra personas_tienda
   + permisos_persona, y si todo está bien renderiza al children con el
   usuario autenticado.
   ────────────────────────────────────────────────────────────────────────── */

const SK_FIN = 'berna_finanzas_session';
...
export default function FinanzasGate({ children }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem(SK_FIN);
      if (stored) {
        const u = JSON.parse(stored);
        if (u && u.id_persona && u.permisos) setUsuario(u);
      }
    } catch {}
    setIniciando(false);
  }, []);
  ...
  const intentarLogin = async (p) => {
    ...
    try {
      ...
      if (!result) {
        ...
        setError('No tienes acceso a Finanzas');
        setPin('');
      } else {
        const seguro = { ...result };
        delete seguro.pin;
        ...
        setUsuario(seguro);
        try { localStorage.setItem(SK_FIN, JSON.stringify(seguro)); } catch {}
      }
    } catch (e) {
      console.error(e);
  ...
  const logout = () => {
    setUsuario(null);
    setPin('');
    try { localStorage.removeItem(SK_FIN); } catch {}
  };
      ...
      <div className="min-h-screen bg-[#fafaf9] flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-sm">
          <div className="flex flex-col items-center mb-10">
            ...
            <h1 className="text-2xl font-semibold text-[#1c1917] tracking-tight">Finanzas</h1>
