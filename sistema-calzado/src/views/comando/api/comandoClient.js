// src/views/comando/api/comandoClient.js
import { supabase } from '../../../api/supabase';
import { verifyPersonaPin } from '../../../lib/pinAuth';

/* ─── Auth ──────────────────────────────────────────────────────────────── */

export async function loginComando(pin) {
  // 1. Obtener personas con permiso 'comando' activo
  ...
  if (!ids.length) {
    const err = new Error('No hay personas con acceso al Modo Comando configurado');
  ...
  for (const row of candidatas || []) {
    ...
    const nivelComando = (permisos || []).find(x => x.recurso === 'comando')?.nivel_acceso;
    const nivelesValidos = ['registrar', 'editar', 'admin'];
    if (!nivelesValidos.includes(nivelComando)) {
      const err = new Error('Esta persona no tiene acceso al Modo Comando');
      err.code = 'NO_PERMISSION';
      throw err;
    }
...
export async function listarCuentasComando() {
  const { data, error } = await supabase.from('v_comando_cuentas').select('*');
  if (error) throw error;
  return data || [];
}

async function obtenerCuentaFinanciera(idCuenta) {
  const { data, error } = await supabase
    .from('cuentas_financieras')
    .select('id_cuenta, nombre, alias, saldo_actual, tipo_cuenta')
...
async function validarSaldoDisponible(idCuenta, monto) {
  const cuenta = await obtenerCuentaFinanciera(idCuenta);
  const saldoRestante = Number(cuenta.saldo_actual) - Number(monto);
  if (cuenta.tipo_cuenta !== 'credito' && saldoRestante < 0) {
    throw new Error(`Saldo insuficiente en ${cuenta.alias || cuenta.nombre || 'la cuenta'}. Transfiere fondos primero.`);
  }
  return cuenta;
}
...
export async function registrarGastoComando({ idCuenta, monto, concepto, idTipo, idPersona }) {
  const cuenta = await validarSaldoDisponible(idCuenta, monto);
...
export async function registrarPagoDeudaComando({ idDeuda, idCuenta, monto, idPersona }) {
  const cuenta = await validarSaldoDisponible(idCuenta, monto);
...
export async function registrarTransferenciaComando({ idCuentaOrigen, idCuentaDestino, monto, concepto, idPersona }) {
  // Insertar dos movimientos: egreso origen + ingreso destino
  const montoN = Number(monto);
  const origen = await validarSaldoDisponible(idCuentaOrigen, montoN);
  const destino = await obtenerCuentaFinanciera(idCuentaDestino);
