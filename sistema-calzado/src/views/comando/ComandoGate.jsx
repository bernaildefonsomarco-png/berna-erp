// Migración única de sesión legacy
const _legacy = localStorage.getItem('berna.rapido.session.v1');
if (_legacy && !localStorage.getItem('berna.comando.session.v1')) {
  localStorage.setItem('berna.comando.session.v1', _legacy);
  localStorage.removeItem('berna.rapido.session.v1');
}

import React, { useState, useEffect } from 'react';
import { loginComando } from './api/comandoClient';

/* ──────────────────────────────────────────────────────────────────────────
   ComandoGate
   Portero del Modo Comando. Valida el PIN y el permiso 'comando'.
   Mismo patrón que FinanzasGate pero con identidad visual de alto contraste.
   ────────────────────────────────────────────────────────────────────────── */

const SK_COMANDO = 'berna.comando.session.v1';
...
function NumPadComando({ value, onChange, onSubmit, loading, maxLength = 4 }) {
  const press = (d) => {
    if (loading) return;
...
export default function ComandoGate({ children }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem(SK_COMANDO);
      if (stored) {
  ...
  const intentarLogin = async (p) => {
    ...
    setError('');
    try {
      const result = await loginComando(p);
      const seguro = { ...result };
      delete seguro.pin;
      delete seguro.pin_hash;
      setUsuario(seguro);
      try { localStorage.setItem(SK_COMANDO, JSON.stringify(seguro)); } catch {}
    } catch (e) {
      const msg =
        e.code === 'INVALID_PIN'    ? 'PIN incorrecto' :
        e.code === 'NO_PERMISSION'  ? 'No tienes acceso al Modo Comando' :
  ...
  const logout = () => {
    setUsuario(null);
    setPin('');
    try { localStorage.removeItem(SK_COMANDO); } catch {}
  };
      ...
      <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6">
        <div className="w-full max-w-sm">
          ...
          <div className="flex flex-col items-center mb-12">
            ...
            <h1 className="text-3xl font-bold text-[#0a0a0a]">Modo Comando</h1>
